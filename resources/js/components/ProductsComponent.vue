<template>
    <div class="page">
        <SearchComponent
            ...
            :keys="fields.filter(item => item.key !== 'actions' && item.key !== 'images' && item.key !== 'category')"
            :isCondition="false"
            :isAction="false"
        />
                    ...
                    <div class="card">
                        ...
                        >
                            ...
                            <template #cell(category)="row">
                                <span v-if="row.item.category && row.item.category.title">
                        ...
                        <b-row class="mb-1 bRowPosition">
                            <b-col class="blockInput">
                                ...
                                <multiselect
                                    ...
                                    :placeholder="__(('variable.category'))"
                                    :showNoResults="false"
                                    :class="[errors['category_id'] ? 'error-border': '', 'x']"
                                >
                                </multiselect>
...
export default {
    ...
    methods: {
        ...
        loadCategories() {
            // assumes categories.index returns {data:[...]} or pagination. If your category index returns paginate, we can fetch large perPage.
            axios.get(route("categories.index", { locale: this.lang }), { params: { perPage: 500 } })
                .then((resp) => {
                    const arr = resp?.data?.data || [];
                    this.categories = arr;
                    this.categoryOptions = [{ value: null, text: "—" }]
        ...
        save() {
            ...
            fd.append(
                "category_id",
                this.form.category_id && this.form.category_id.value != null ? this.form.category_id.value : ""
